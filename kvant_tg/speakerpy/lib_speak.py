...
class SpeakerBase:
    ...
    def _init_models(
        ...
        models_list_file = os.path.join(os.path.dirname(__file__), '..', 'latest_silero_models.yml')
        ...
        model_conf = models.tts_models[language][model_id].latest

        # Определяем формат модели: package (.pt) или jit
        if hasattr(model_conf, 'package'):
            ...
            # PackageImporter не поддерживает Unicode-пути на Windows (torch C++ fopen),
            # а передача BytesIO ломает десериализацию (модель теряет методы).
            # Обходное решение: временно переходим в каталог модели и открываем по имени файла.
            _prev_cwd = os.getcwd()
            try:
                os.chdir(model_dir)
                imp = torch_package.PackageImporter(os.path.basename(model_path))
                _model = imp.load_pickle("tts_models", "model")
            finally:
                os.chdir(_prev_cwd)
        else:
            from silero.utils import init_jit_model_tts
            _model = init_jit_model_tts(model_conf.jit)
    ...
    def to_mp3(
        ...
        *,
        ...
        list_path_to_audio: list[pathlib.Path] = list(
            self._chunks_synthes(sl_text, audio_dir, _speed, sample_rate, put_accent=put_accent, put_yo=put_yo)
        )
        ###
        #
